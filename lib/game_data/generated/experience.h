/**********************************************************
         AUTO GENERATED FILE BY sox_code_generator
                   DO NOT EDIT THIS FILE
**********************************************************/
#pragma once
#include <cstdint>
#include <string>
#include <vector>
#include <unordered_map>
#include <filesystem>
#include "lib/game_data/game_data_table.h"

namespace cebreiro { class StreamReader; }

namespace cebreiro::gamedata
{
	struct Experience
	{
		Experience() = default;
		explicit Experience(StreamReader& reader);

		int32_t index = 0;
		int32_t cLvExp = 0;
		int32_t jLvExp = 0;
	};

	class ExperienceTable : public GameDataTable
	{
	public:
		 static constexpr int64_t INDEX = 12;

	public:
		void LoadFromFile(const std::filesystem::path& path);

		auto Find(int32_t index) const -> const Experience*;
		auto Get() const -> const std::vector<Experience>&;

	private:
		std::vector<Experience> _vector;
		std::unordered_map<int32_t, const Experience*> _umap;
	};
}
